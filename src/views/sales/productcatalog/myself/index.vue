  ...
  <div class="product-catalog-myself-container">
              ...
              <el-tooltip class="item" effect="dark" content="点击展开操作列表，可执行对应操作" placement="top-start">
                <el-dropdown split-button type="primary" placement="bottom-end" trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown" trigger="click">
                    <el-dropdown-item><el-button type="success" icon="el-icon-check" size="mini" round @click="handleCheck">上架货品</el-button></el-dropdown-item>
                    <el-dropdown-item><el-button type="danger" icon="el-icon-close" size="mini" round @click="handleReject">下架货品</el-button></el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </el-tooltip>
              <el-tooltip class="item" effect="dark" content="点击选中所有筛选出的货品" placement="top-start">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button type="success" size="mini" @click="handleOnSale(scope.row)">上架</el-button>
            <el-button type="danger" size="mini" @click="handleOffSale(scope.row)">下架</el-button>
          </template>
        </el-table-column>
        <el-table-column
          label="备注"
    ...
    >
      ...
      >
        <el-card class="box-card">
          ...
          <el-row :gutter="20">
            <el-col :span="8"><el-form-item label="经销商" prop="company">
              <template>
                <el-select
            ...
            <el-col :span="8"><el-form-item label="货品" prop="goods">
              <template>
                <el-select
          ...
          <el-row :gutter="20">
            <el-col :span="8"><el-form-item label="销售单价" prop="price">
              <el-input v-model="formAdd.price" placeholder="销售单价" />
            </el-form-item></el-col>
          </el-row>
          <el-row :gutter="20">
        ...
        <div class="handleFormEdit">
          ...
          >
            <el-card class="box-card">
              ...
              <el-row :gutter="20">
                <el-col :span="8"><el-form-item label="销售单价" prop="price">
                  <el-input v-model="formEdit.price" placeholder="销售单价" />
                </el-form-item></el-col>
              </el-row>
              <el-row :gutter="20">
                <el-col :span="8"><el-form-item label="备注" prop="memo">
...
<script>
import {
  getProductCatalogMyselfList,
  createProductCatalogMyself,
  updateProductCatalogMyself,
  exportProductCatalogMyself,
  checkProductCatalogMyself,
  rejectProductCatalogMyself
} from '@/api/sales/productcatalog/myself'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getProductCatalogMyselfList(this.params).then(
        res => {
    ...
    handleEdit(values) {
      ...
      delete this.formEdit.order_status
    },
    // 提交编辑完成的数据
    handleSubmitEdit() {
      this.$refs.handleFormEdit.validate(valid => {
        ...
        let attrStr
        console.log(data)
        const transFieldStr = ['department']
        for (attrStr in transFieldStr) {
          data[transFieldStr[attrStr]] = data[transFieldStr[attrStr]].id
        }
        console.log(data)
        updateProductCatalogMyself(id, data).then(
          () => {
    ...
    // 递交添加
    handleSubmitAdd() {
      this.formAdd.department = this.$store.state.user.department.id
      createProductCatalogMyself(this.formAdd).then(
        () => {
          this.fetchData()
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkProductCatalogMyself(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectProductCatalogMyself(this.params).then(
                res => {
                  if (res.data.success !== 0) {
                    this.$notify({
                      title: '取消成功',
    ...
    handleOnSale(data) {
      ...
      id = data.id
      updateProductCatalogMyself(id, edit_data).then(
        () => {
          this.dialogVisibleEdit = false
          this.fetchData()
        }

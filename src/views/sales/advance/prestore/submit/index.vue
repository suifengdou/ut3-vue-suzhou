        ...
        <el-col :span="7" class="titleBar">
          <div class="grid-content bg-purple">
            <el-tooltip class="item" effect="dark" content="点击弹出新建界面" placement="top-start">
              <el-button type="primary" @click="add">新增预存单</el-button>
            </el-tooltip>
          </div>
    ...
    >
      ...
      >
        <el-card class="box-card">
          ...
          <el-row :gutter="20"><el-col :span="16"><el-form-item label="流水号" prop="bank_sn">
            <el-input v-model="formAdd.bank_sn" placeholder="请输入流水号" />
          </el-form-item></el-col></el-row>
          <el-row :gutter="20"><el-col :span="16"><el-form-item label="存入金额" prop="amount">
            <el-input v-model="formAdd.amount" type="number" placeholder="请输入存入金额" />
          </el-form-item></el-col></el-row>
          <el-row :gutter="20"><el-col :span="16"><el-form-item label="备注" prop="memorandum">
            <el-input v-model="formAdd.memorandum" placeholder="请输入备注" />
          </el-form-item></el-col></el-row>
        </el-card>
        <el-card class="box-card">
          <el-row :gutter="20">
            <el-col :span="12" :offset="10"><el-form-item size="large">
        ...
        <div class="handleFormEdit">
          ...
          >
            <el-card class="box-card">
              ...
              <el-row :gutter="20"><el-col :span="16"><el-form-item label="流水号" prop="bank_sn">
                <el-input v-model="formEdit.bank_sn" placeholder="请输入流水号" />
              </el-form-item></el-col></el-row>
              <el-row :gutter="20"><el-col :span="16"><el-form-item label="存入金额" prop="amount">
                <el-input v-model="formEdit.amount" type="number" placeholder="请输入存入金额" />
              </el-form-item></el-col></el-row>
              <el-row :gutter="20"><el-col :span="16"><el-form-item label="备注" prop="memorandum">
...
<script>
import { getPrestoreSubmitList, createPrestoreSubmit, updatePrestoreSubmit, checkPrestoreSubmit, rejectPrestoreSubmit } from '@/api/sales/advance/prestore'
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getPrestoreSubmitList(this.params).then(
        res => {
    ...
    handleSubmitAdd() {
      ...
      createPrestoreSubmit(this.formAdd).then(
        () => {
          this.fetchData()
    ...
    handleSubmitEdit() {
      this.$refs.FormEdit.validate(valid => {
        ...
        const data = {
          order_id: this.formEdit.order_id,
          bank_sn: this.formEdit.bank_sn,
          amount: this.formEdit.amount,
          memorandum: this.formEdit.memorandum
        }
        updatePrestoreSubmit(id, data).then(
          () => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkPrestoreSubmit(this.params).then(
          res => {
    ...
    handleReject() {
      ...
      this.$msgbox({
        ...
        message: h('p', null, [
          ...
          h('hr', null, ''),
          h('span', null, '取消工单即为此源单号的开票申请彻底取消！无法再次用流水号创建开票申请，请慎重选择！'),
          h('hr', null, '')
        ]),
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            if (this.params.allSelectTag === 1) {
              rejectPrestoreSubmit(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',

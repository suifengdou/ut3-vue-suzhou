...
import {
  ...
  updateNationality,
  exportNationality,
  excelImportNationality
} from '@/api/utils/geography/nationality'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getNationalityList(this.params).then(
        res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportNationality(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportNationality(this.params).then(
              res => {
    ...
    handleSubmitAdd() {
      ...
      createNationality(this.formAdd).then(
        () => {
          this.fetchData()

<template>
  <div class="batchdetail-settle-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.batch_order.order_id }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="OA单号"
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.batch_order.oa_order_id }}</span>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.batch_order.shop }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="单据类型"
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.batch_order.order_category }}</span>
          </template>
        </el-table-column>
...
import {
  getBCDetailList,
  exportBCDetail,
} from '@/api/dfc/compensation/batchdetail/bcdetail'
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            exportBCDetail(this.params).then(
              res => {
                res.data = res.data.map(item => {
                  return {
                    批次号: item.batch_order.order_id,
                    订单号: item.order_id,
                    店铺: item.batch_order.shop,
                    网名: item.nickname,
                    姓名: item.name,
                    支付宝: item.alipay_id,
                    单据类型: item.batch_order.order_category,

  ...
  <div class="batchdetail-settle-container">
              ...
              <el-tooltip class="item" effect="dark" content="点击展开操作列表，可执行对应操作" placement="top-start">
                <el-dropdown split-button type="primary" placement="bottom-end" trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown" trigger="click">
                    <el-dropdown-item><el-button type="success" icon="el-icon-close" size="mini" round @click="handleReset">重置</el-button></el-dropdown-item>
        ...
        <el-col :span="5" class="titleBar">
          <div class="grid-content bg-purple">
            <el-tooltip class="item" effect="dark" content="快捷搜索" placement="top-start">
              <el-input v-model="params.batch_order__oa_order_id" class="grid-content bg-purple" placeholder="请输入OA单号" @keyup.enter.native="fetchData">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button size="mini" type="success" @click="confirmPayment(scope.row)">已付</el-button>
          </template>
        </el-table-column>
        <el-table-column
          label="重置按钮"
        >
          <template slot-scope="scope">
            <el-button size="mini" type="danger" @click="resetPayment(scope.row)">重置</el-button>
          </template>
        </el-table-column>
        <el-table-column
          label="处理标签"
        ...
        >
          <template slot-scope="scope">
            <el-select
              v-model="scope.row.process_tag.id"
              reserve-keyword
              placeholder="请选择处理标签"
              @change="confirmProcess(scope.row)"
            >
              <el-option
...
<script>
import {
  getBCDetailSettleList,
  createBCDetailSettle,
  updateBCDetailSettle,
  exportBCDetailSettle,
  excelImportBCDetailSettle,
  checkBCDetailSettle,
  resetBCDetailSettle
} from '@/api/dfc/compensation/batchdetail/bcdetailsettle'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getBCDetailSettleList(this.params).then(
        res => {
    ...
    confirmCategory(row) {
      ...
      console.log(data)
      updateBCDetailSettle(id, data).then(
        () => {
          this.$notify({
            title: '修改成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            exportBCDetailSettle(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkBCDetailSettle(this.params).then(
          res => {
    ...
    handleReset() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetBCDetailSettle(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '重置成功',
    ...
    confirmPayment(row) {
      const { id, ...details } = row
      const data = {
        paid_amount: details.compensation,
        is_payment: true
      }
      console.log(data)
      updateBCDetailSettle(id, data).then(

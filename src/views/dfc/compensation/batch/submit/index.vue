  ...
  <div class="manualorder-submit-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.oa_order_id }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="单据类型"
          ...
          >
            <el-card class="box-card">
              ...
              <el-row :gutter="20">
                <el-col :span="8"><el-form-item label="单据类型" prop="order_category">
                  <div v-if="formEdit.order_category != undefined">
            ...
            <el-card class="box-card">
              ...
              <el-table
                ref="tableEdit"
                border
                :data="orderDetailListEdit"
                :row-class-name="rowClassName"
              >
                <el-table-column label="序号" align="center" prop="xh" width="50">
                  <template slot-scope="scope">
                    <span>{{ orderDetailListEdit[scope.row.xh-1].xh }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="收款人" width="250" prop="name">
                  <template slot-scope="scope">
                    <span>{{ orderDetailListEdit[scope.row.xh-1].name }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="支付宝" width="250" prop="alipay_id">
                  <template slot-scope="scope">
                    <span>{{ orderDetailListEdit[scope.row.xh-1].alipay_id }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="货品" width="250" prop="goods_name">
                  <template slot-scope="scope">
                    <span>{{ orderDetailListEdit[scope.row.xh-1].goods_name }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="补偿金额" width="250" prop="compensation">
                  <template slot-scope="scope">
                    <span>{{ orderDetailListEdit[scope.row.xh-1].compensation }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="补偿金额" width="250" prop="compensation">
                  <template slot-scope="scope">
                    <span>{{ orderDetailListEdit[scope.row.xh-1].paid_amount }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="备注" width="250" prop="memorandum">
                  <template slot-scope="scope">
                    <span>{{ orderDetailListEdit[scope.row.xh-1].memorandum }}</span>
                  </template>
                </el-table-column>
              </el-table>
            </el-card>
...
<script>
  import {
    getBatchCompensationSubmitList,
    createBatchCompensationSubmit,
    updateBatchCompensationSubmit,
    exportBatchCompensationSubmit,
    excelImportBatchCompensationSubmit,
    checkBatchCompensationSubmit,
    rejectBatchCompensationSubmit
  } from '@/api/dfc/compensation/batch/batchcompensationsubmit'
  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        getBatchCompensationSubmitList(this.params).then(
          res => {
      ...
      handleEdit(values) {
        ...
        this.orderDetailListEdit = this.formEdit.compensation_details
      ...
      handleCheck() {
        ...
        if (this.params.allSelectTag === 1) {
          checkBatchCompensationSubmit(this.params).then(
            res => {
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              if (this.params.allSelectTag === 1) {
                rejectBatchCompensationSubmit(this.params).then(
                  res => {
                    if (res.data.successful !== 0) {
                      this.$notify({
                        title: '取消成功',
      ...
      handelDoubleClick(row, column, cell, event) {
        if (column.property === 'oa_order_id') {
          this.handleOA(row)
        }
      },
      // 修改备注
      handleOA(row) {
        this.$prompt('请输入反馈内容', '添加反馈', {
          ...
          ({ value }) => {
            ...
            updateBatchCompensationSubmit(id, data).then(
              () => {
                this.$notify({
                  title: '修改成功',
                  type: 'success',

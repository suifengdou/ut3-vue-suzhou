  ...
  <div class="componentproject-prepare-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        <el-table-column
          label="整机版本编码"
          prop="subunits_project"
          width="120px"
          sortable="custom"
          :sort-orders="['ascending','descending']"
        >
          <template slot-scope="scope">
            <span>{{ scope.row.subunits_project.unit_code }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="子项版本编码"
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.subunits_project.subunit_code }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="子项版本名称"
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.subunits_project.subunit_name }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="类型"
...
<script>
import {
  getComponentProjectPrepare,
  updateComponentProjectPrepare,
  exportComponentProjectPrepare,
  excelImportComponentProjectPrepare,
  checkComponentProjectPrepare,
  rejectComponentProjectPrepare,
  photoImportComponentProjectPrepare,
  setTagComponentProjectPrepare,
  resetTagComponentProjectPrepare
} from '@/api/project/componentproject/prepare'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getComponentProjectPrepare(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportComponentProjectPrepare(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportComponentProjectPrepare(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportComponentProjectPrepare(this.params).then(
              res => {
    ...
    handleSetTag() {
      ...
      if (this.params.allSelectTag === 1) {
        setTagComponentProjectPrepare(this.params).then(
          res => {
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagComponentProjectPrepare(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '设置成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              checkComponentProjectPrepare(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '确认成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectComponentProjectPrepare(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handleMemo(row) {
      this.$prompt('请输入备注', '添加备注', {
        ...
        ({ value }) => {
          ...
          updateComponentProjectPrepare(id, data).then(
            () => {
              this.$notify({
                title: '修改成功',
                type: 'success',

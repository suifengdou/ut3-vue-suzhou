<template>
  <div class="phototypeexecuteprojectdetails-manage-container">
...
<script>
import {
  getPhototypeExecuteProjectDetails,
  exportPhototypeExecuteProjectDetails,
  getLogPhototypeExecuteProjectDetails,
  getFileDetailsPhototypeExecuteProjectDetails
} from '@/api/project/phototypeexecuteproject/details/manage'
...
export default {
  name: 'managePhototypeExecuteProjectDetails',
  ...
  methods: {
    fetchData() {
      ...
      getPhototypeExecuteProjectDetails(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImporPhototypeProjectDetails(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportPhototypeProjectDetails(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportPhototypeExecuteProjectDetails(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkPhototypeProjectDetails(this.params).then(
          res => {
    ...
    handleSetConfirm() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        setTagPhototypeProjectDetails(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '审核成功',
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagPhototypeProjectDetails(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '审核成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPhototypeProjectDetails(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    submitPrice(row) {
      ...
      if (!(!isNaN(parseFloat(price)) && isFinite(price))) {
        ...
        updatePhototypeProjectDetails(id, data).then(
          () => {

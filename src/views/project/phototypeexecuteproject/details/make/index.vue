<template>
  <div class="phototypeexecuteprojectdetails-make-container">
...
<script>
import {
  getPhototypeExecuteProjectDetailsMake,
  updatePhototypeExecuteProjectDetailsMake,
  exportPhototypeExecuteProjectDetailsMake,
  excelImportPhototypeExecuteProjectDetailsMake,
  checkPhototypeExecuteProjectDetailsMake,
  rejectPhototypeExecuteProjectDetailsMake,
  photoImportPhototypeExecuteProjectDetailsMake,
  setTagPhototypeExecuteProjectDetailsMake,
  resetTagPhototypeExecuteProjectDetailsMake
} from '@/api/project/phototypeexecuteproject/details/make'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getPhototypeExecuteProjectDetailsMake(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImporPhototypeExecuteProjectDetailsMake(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportPhototypeExecuteProjectDetailsMake(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportPhototypeExecuteProjectDetailsMake(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkPhototypeExecuteProjectDetailsMake(this.params).then(
          res => {
    ...
    handleSetConfirm() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        setTagPhototypeExecuteProjectDetailsMake(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '审核成功',
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagPhototypeExecuteProjectDetailsMake(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '审核成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPhototypeExecuteProjectDetailsMake(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    submitPrice(row) {
      ...
      if (!(!isNaN(parseFloat(price)) && isFinite(price))) {
        ...
        updatePhototypeExecuteProjectDetailsMake(id, data).then(
          () => {

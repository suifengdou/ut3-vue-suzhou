<template>
  <div class="phototypeproject-prepare-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-select
              v-model="scope.row.phototype_supplier"
              filterable
              default-first-option
              remote
              reserve-keyword
              placeholder="请搜索并选择供应商"
              :remote-method="remoteMethodPhototypeSupplier"
              @change="confirmSupplier(scope.row)"
            >
              <el-option
                v-for="item in optionsPhototypeSupplier"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
...
<script>
import {
  getPhototypeExecuteProjectPrepare,
  updatePhototypeExecuteProjectPrepare,
  exportPhototypeExecuteProjectPrepare,
  excelImportPhototypeExecuteProjectPrepare,
  checkPhototypeExecuteProjectPrepare,
  rejectPhototypeExecuteProjectPrepare,
  photoImportPhototypeExecuteProjectPrepare,
  setTagPhototypeExecuteProjectPrepare,
  resetTagPhototypeExecuteProjectPrepare,
} from '@/api/project/phototypeexecuteproject/project/prepare'
import { getLogPhototypeExecuteProject, getFileDetailsPhototypeExecuteProject } from '@/api/project/phototypeexecuteproject/project/manage'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getPhototypeExecuteProjectPrepare(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportPhototypeExecuteProjectPrepare(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
    ...
    logView(userValue) {
      ...
      getLogPhototypeExecuteProject(data).then(
        res => {
          this.$notify({
            title: '查询成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportPhototypeExecuteProjectPrepare(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportPhototypeExecuteProjectPrepare(this.params).then(
              res => {
    ...
    handleSetTag() {
      ...
      if (this.params.allSelectTag === 1) {
        setTagPhototypeExecuteProjectPrepare(this.params).then(
          res => {
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagPhototypeExecuteProjectPrepare(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '设置成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              checkPhototypeExecuteProjectPrepare(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '确认成功',
    ...
    handleReject() {
      ...
      this.$msgbox({
        ...
        message: h('p', null, [
          h('h3', { style: 'color: teal' }, '特别注意：'),
          h('hr', null, ''),
          h('span', null, '驳回项目，将会把所有项目明细驳回到准备状态。如果项目单不是准备状态则不可驳回！'),
          h('b',  { style: 'color: red' }, '执行单相关内容全部会被取消！'),
          h('hr', null, '')
        ]),
        showCancelButton: true,
        confirmButtonText: '确定',
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPhototypeExecuteProjectPrepare(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handleMemo(row) {
      this.$prompt('请输入备注', '添加备注', {
        ...
        ({ value }) => {
          ...
          updatePhototypeExecuteProjectPrepare(id, data).then(
            () => {
              this.$notify({
                title: '修改成功',
                type: 'success',

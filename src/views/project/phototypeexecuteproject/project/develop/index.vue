<template>
  <div class="phototypeproject-develop-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.phototype_supplier.name }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="产品线"
...
<script>
import {
  getPhototypeExecuteProjectDevelop,
  updatePhototypeExecuteProjectDevelop,
  exportPhototypeExecuteProjectDevelop,
  excelImportPhototypeExecuteProjectDevelop,
  checkPhototypeExecuteProjectDevelop,
  rejectPhototypeExecuteProjectDevelop,
  photoImportPhototypeExecuteProjectDevelop,
  setTagPhototypeExecuteProjectDevelop,
  resetTagPhototypeExecuteProjectDevelop,
} from '@/api/project/phototypeexecuteproject/project/develop'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getPhototypeExecuteProjectDevelop(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportPhototypeExecuteProjectDevelop(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportPhototypeExecuteProjectDevelop(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportPhototypeExecuteProjectDevelop(this.params).then(
              res => {
    ...
    handleSetTag() {
      ...
      if (this.params.allSelectTag === 1) {
        setTagPhototypeExecuteProjectDevelop(this.params).then(
          res => {
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagPhototypeExecuteProjectDevelop(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '设置成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              checkPhototypeExecuteProjectDevelop(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '确认成功',
    ...
    handleReject() {
      ...
      this.$msgbox({
        ...
        message: h('p', null, [
          h('h3', { style: 'color: teal' }, '特别注意：'),
          h('hr', null, ''),
          h('span', null, '驳回项目，此项目下的结算单会被取消。'),
          h('b',  { style: 'color: red' }, '如果结算单已审核，则无法驳回！'),
          h('hr', null, '')
        ]),
        showCancelButton: true,
        confirmButtonText: '确定',
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPhototypeExecuteProjectDevelop(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handleMemo(row) {
      this.$prompt('请输入备注', '添加备注', {
        ...
        ({ value }) => {
          ...
          updatePhototypeExecuteProjectDevelop(id, data).then(
            () => {
              this.$notify({
                title: '修改成功',
                type: 'success',

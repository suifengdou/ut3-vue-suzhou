<template>
  <div class="phototyexecutepeproject-manage-container">
...
<script>
import {
  getPhototypeExecuteProject,
  exportPhototypeExecuteProject,
  getLogPhototypeExecuteProject,
  getFileDetailsPhototypeExecuteProject
} from '@/api/project/phototypeexecuteproject/project/manage'
...
export default {
  name: 'managePhototypeExecuteProject',
  ...
  methods: {
    fetchData() {
      ...
      getPhototypeExecuteProject(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportPhototypeExecuteProject(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportPhototypeExecuteProject(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportPhototypeExecuteProject(this.params).then(
              res => {
    ...
    handleSetTag() {
      ...
      if (this.params.allSelectTag === 1) {
        setTagPhototypeExecuteProject(this.params).then(
          res => {
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagPhototypeExecuteProject(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '设置成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              checkPhototypeExecuteProject(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '确认成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPhototypeExecuteProject(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handleMemo(row) {
      this.$prompt('请输入备注', '添加备注', {
        ...
        ({ value }) => {
          ...
          updatePhototypeExecuteProject(id, data).then(
            () => {
              this.$notify({
                title: '修改成功',
                type: 'success',

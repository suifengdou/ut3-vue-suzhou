...
import {
  getWorkOrderMyself,
  createWorkOrderMyself,
  updateWorkOrderMyself,
  exportWorkOrderMyself,
  excelImportWorkOrderMyself,
  photoImportWorkOrderMyself,
  checkWorkOrderMyself,
  createServiceWorkOrderMyself,
  rejectWorkOrderMyself,
} from '@/api/wop/satisfaction/satisfaction/myself'
import {
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getWorkOrderMyself(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportWorkOrderMyself(importformData, config).then(
        res => {
          console.log(res)
          this.$notify({
            title: '导入结果',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportWorkOrderMyself(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkWorkOrderMyself(this.params).then(
          res => {
    ...
    handleCreateService() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        createServiceWorkOrderMyself(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '创建成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectWorkOrderMyself(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',

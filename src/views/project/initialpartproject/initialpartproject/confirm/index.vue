<template>
  <div class="initialpartproject-confirm-container">
...
<script>
import {
  getInitialPartProjectConfirm,
  updateInitialPartProjectConfirm,
  exportInitialPartProjectConfirm,
  checkInitialPartProjectConfirm,
  rejectInitialPartProjectConfirm,
  photoImportInitialPartProjectConfirm
} from '@/api/project/initialpartproject/initialpartproject/confirm'
import { getLogInitialPartProjectManage, getFileDetailsInitialPartProjectManage } from '@/api/project/initialpartproject/initialpartproject/manage'
...
export default {
  name: 'confirmInitialPartProjectProject',
  ...
  methods: {
    fetchData() {
      ...
      getInitialPartProjectConfirm(this.params).then(
        res => {
    ...
    handleConfirmEdit() {
      this.$refs.handleFormEdit.validate(valid => {
        ...
        console.log(data)
        updateInitialPartProjectConfirm(id, data).then(
          () => {
            this.$notify({
              title: '更新成功',
              type: 'success',
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportInitialPartProjectConfirm(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
    ...
    logView(userValue) {
      ...
      getLogInitialPartProjectManage(data).then(
        res => {
          this.$notify({
            title: '查询成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            exportInitialPartProjectConfirm(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkInitialPartProjectConfirm(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectInitialPartProjectConfirm(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handelDoubleClick(row, column, cell, event) {
      ...
      } else if (column.property === 'group_code') {
        this.handleGroupCode(row)
      }
    },
    ...
    handleGroupCode(row) {
      this.$prompt('请输入分组序号', '添加分组序号', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        inputValue: row.group_code,
        inputErrorMessage: '输入错误或者为空',
        inputValidator: (value) => {
          let re = /^[0-9-]+.?[0-9]*/
          if(!value) {

<template>
  <div class="phototypeprojectdetails-purchase-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-input
              v-model="scope.row.group_code"
              placeholder="分组标识"
              @change="submitGroupcode(scope.row)"
            />
          </template>
        </el-table-column>
        <el-table-column
      ...
      <template>
        <div class="handleFormEdit">
          ...
          >
            ...
            <el-row :gutter="20">
              <el-col :span="8"><el-form-item label="原初物料" prop="name">
                <dif v-if="formEdit.name !== undefined">
                  <span>{{ formEdit.name }}</span>
                </dif>
              </el-form-item></el-col>
              <el-col :span="8"><el-form-item label="原初物料编码" prop="code">
                <dif v-if="formEdit.code !== undefined">
                  <span>{{ formEdit.code }}</span>
                </dif>
              </el-form-item></el-col>
            </el-row>
            <el-row :gutter="20">
              <el-col :span="8"><el-form-item label="单据类型" prop="order_category">
                <dif v-if="formEdit.component_project !== undefined">
            ...
            <el-row :gutter="20">
              <el-col :span="8"><el-form-item label="组项项目" prop="component_project">
                <template>
                  <dif v-if="formEdit.component_project !== undefined">
                    <span>{{ formEdit.component_project.name }}</span>
                  </dif>
                </template>
              </el-form-item></el-col>
              <el-col :span="8"><el-form-item label="类型" prop="category">
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="8"><el-form-item label="分组标识" prop="group_code">
                <el-input v-model="formEdit.group_code" placeholder="请输入分组标识" />
              </el-form-item></el-col>
            </el-row>
            <el-row :gutter="20">
              ...
              <el-row :gutter="20">
                <el-col :span="16" :offset="8"><el-form-item size="large">
                  <div class="btn-warpper">
                    ...
                    <el-button type="primary" @click="handleConfirmEdit">立即保存</el-button>
                  </div>
                </el-form-item></el-col>
              </el-row>
            </el-card>
...
<script>
import {
  getPhototypeProjectDetailsPurchase,
  updatePhototypeProjectDetailsPurchase,
  exportPhototypeProjectDetailsPurchase,
  excelImporPhototypeProjectDetailsPurchase,
  checkPhototypeProjectDetailsPurchase,
  rejectPhototypeProjectDetailsPurchase,
  photoImportPhototypeProjectDetailsPurchase,
  setTagPhototypeProjectDetailsPurchase,
  resetTagPhototypeProjectDetailsPurchase
} from '@/api/project/phototypeproject/details/purchase'
import { getLogPhototypeProjectDetails, getFileDetailsPhototypeProjectDetails } from '@/api/project/phototypeproject/details/manage'
...
export default {
  name: 'purchasePhototypeProjectDetails',
  ...
  methods: {
    fetchData() {
      ...
      getPhototypeProjectDetailsPurchase(this.params).then(
        res => {
    ...
    handleConfirmEdit() {
      this.$refs.handleFormEdit.validate(valid => {
        ...
        let attrStr
        console.log(data)
        const transFieldStr = ['order_category', 'component_project', 'creator', 'mistake_tag']
        if (data.material === -1) {
        ...
        console.log(data)
        updatePhototypeProjectDetailsPurchase(id, data).then(
          () => {
            this.$notify({
              title: '更新成功',
              type: 'success',
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportPhototypeProjectDetailsPurchase(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
    ...
    handlePhotoView(userValue) {
      ...
      getFileDetailsInitialPartProjectManage(data).then(
        res => {
          this.$notify({
            title: '查询成功',
            type: 'success',
    ...
    logView(userValue) {
      ...
      getLogPhototypeProjectDetails(data).then(
        res => {
          this.$notify({
            title: '查询成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            exportPhototypeProjectDetailsPurchase(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkPhototypeProjectDetailsPurchase(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPhototypeProjectDetailsPurchase(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handelDoubleClick(row, column, cell, event) {
      if (column.property === 'diagram_number') {
        this.handleDiagramNumber(row)
      } else if (column.property === 'specification') {
        this.handleSpeification(row)
      } else if (column.property === 'technology') {
        this.handleTechnology(row)
      } else if (column.property === 'material_color_number') {
        this.handleMaterialColorNumber(row)
      } else if (column.property === 'quantity') {
        this.handleQuantity(row)
      } else if (column.property === 'weight') {
        this.handleWeight(row)
      } 
    },
    handleDiagramNumber(row) {
      this.$prompt('请输入爆炸图号', '添加爆炸图号', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        inputValue: row.diagram_number,
    ...
    handleSpeification(row) {
      this.$prompt('请输入规格', '添加规格', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
    ...
    handleTechnology(row) {
      this.$prompt('请输入工艺', '添加工艺', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        inputValue: row.technology,
        inputErrorMessage: '输入不能为空',
        inputValidator: (value) => {
        if(!value) {
          return '输入不能为空';
    ...
    handleMaterialColorNumber(row) {
      this.$prompt('请输入材料色号', '添加材料色号', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        inputValue: row.material_color_number,
        inputErrorMessage: '输入不能为空',
        inputValidator: (value) => {
        if(!value) {
          return '输入不能为空';
    ...
    submitGroupcode(row) {
      ...
      // if (!(!isNaN(parseFloat(group_code)) && isFinite(group_code))) {
      //   this.$notify({
      //     title: '异常错误详情',
      //     message: '只能输入数字分组',
      //     type: 'error',

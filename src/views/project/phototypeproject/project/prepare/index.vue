  ...
  <div class="phototypeproject-prepare-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-select v-model="scope.row.category.id" placeholder="标签" @change="confirmCategory(scope.row)">
              <el-option
                v-for="item in optionsCategory"
                :key="item.value"
                :label="item.label"
...
<script>
import {
  getPhototypeProjectPrepare,
  updatePhototypeProjectPrepare,
  exportPhototypeProjectPrepare,
  excelImportPhototypeProjectPrepare,
  checkPhototypeProjectPrepare,
  rejectPhototypeProjectPrepare,
  photoImportPhototypeProjectPrepare,
  setTagPhototypeProjectPrepare,
  resetTagPhototypeProjectPrepare
} from '@/api/project/phototypeproject/project/prepare'
import { getLogPhototypeProject, getFileDetailsPhototypeProject } from '@/api/project/phototypeproject/project/manage'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getPhototypeProjectPrepare(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportPhototypeProjectPrepare(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
    ...
    handlePhotoView(userValue) {
      ...
      getFileDetailsPhototypeProject(data).then(
        res => {
          this.$notify({
            title: '查询成功',
            type: 'success',
    ...
    logView(userValue) {
      ...
      getLogPhototypeProject(data).then(
        res => {
          this.$notify({
            title: '查询成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportPhototypeProjectPrepare(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportPhototypeProjectPrepare(this.params).then(
              res => {
    ...
    handleSetTag() {
      ...
      if (this.params.allSelectTag === 1) {
        setTagPhototypeProjectPrepare(this.params).then(
          res => {
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagPhototypeProjectPrepare(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '设置成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              checkPhototypeProjectPrepare(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '确认成功',
    ...
    handleReject() {
      ...
      this.$msgbox({
        ...
        message: h('p', null, [
          h('h3', { style: 'color: teal' }, '特别注意：'),
          h('hr', null, ''),
          h('span', null, '驳回项目，系统会清楚所有项目明细并取消此项目。'),
          h('b',  { style: 'color: red' }, '一旦取消无法恢复！'),
          h('hr', null, '')
        ]),
        showCancelButton: true,
        confirmButtonText: '确定',
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPhototypeProjectPrepare(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handleMemo(row) {
      this.$prompt('请输入备注', '添加备注', {
        ...
        ({ value }) => {
          ...
          updatePhototypeProjectPrepare(id, data).then(
            () => {
              this.$notify({
                title: '修改成功',
                type: 'success',

        ...
        <el-col :span="7" class="titleBar">
          <div class="grid-content bg-purple">
            <el-tooltip class="item" effect="dark" content="点击弹出新建界面" placement="top-start">
              <el-button type="primary" @click="add">新增原始整机项目</el-button>
            </el-tooltip>
          </div>
    ...
    >
      ...
      >
        <el-card class="box-card">
          ...
          <el-row :gutter="20">
            <el-col :span="8"><el-form-item label="后缀" prop="suffix">
              <el-input v-model="formAdd.suffix" placeholder="请输入后缀" />
            </el-form-item></el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="8"><el-form-item label="产品线" prop="product_line">
              <template>
                <el-select
                  v-model="formAdd.product_line"
                  filterable
                  default-first-option
                  remote
                  reserve-keyword
                  placeholder="产品线一旦创建不可更改！"
                  :remote-method="remoteMethodProductLine"
                >
                  <el-option
                    v-for="item in optionsProductLine"
          ...
          <el-row :gutter="20">
            <el-col :span="8"><el-form-item label="级别" prop="level">
              <el-select v-model="formAdd.level" placeholder="请选择级别">
                <el-option
                  v-for="item in optionsLevel"
                  :key="item.value"
                  :label="item.label"
          ...
          <el-row :gutter="20">
            <el-col :span="6"><el-form-item label="是否自动设置排序" prop="auto_set_number">
              <template slot-scope="scope">
                <el-switch
                  v-model="formAdd.auto_set_number"
                  active-color="#13ce66"
                  inactive-color="#ff4949"
                />
              </template>
            </el-form-item></el-col>
            <el-col :span="6"><el-form-item label="是否补录" prop="is_supplement">
              <template slot-scope="scope">
                <el-switch
                  v-model="formAdd.is_supplement"
                  active-color="#13ce66"
                  inactive-color="#ff4949"
                />
              </template>
          ...
          <el-row :gutter="20">
            <el-col :span="8"><el-form-item label="系列内排序" prop="serial_number">
              <el-input v-model="formAdd.serial_number" placeholder="请输入系列内排序" />
            </el-form-item></el-col>
            <el-col :span="8"><el-form-item label="整机排序" prop="unit_number">
              <el-input v-model="formAdd.unit_number" placeholder="请输入整机排序" />
            </el-form-item></el-col>
          </el-row>
        </el-card>
        ...
        <div class="handleFormEdit">
          ...
          >
            <el-card class="box-card">
              ...
              <el-row :gutter="20">
                <el-col :span="8"><el-form-item label="后缀" prop="suffix">
                  <el-input v-model="formEdit.suffix" placeholder="请输入后缀" />
                </el-form-item></el-col>
                <el-col :span="8"><el-form-item label="是否修改后缀" prop="is_set_suffix">
                  <template slot-scope="scope">
                    <el-switch
                      v-model="formAdd.is_set_suffix"
                      active-color="#13ce66"
                      inactive-color="#ff4949"
                      active-text="修改后缀名必须打开开关，否则修改无效"
                    />
                  </template>
                </el-form-item></el-col>
              </el-row>
              <el-row :gutter="20">
                <el-col :span="8"><el-form-item label="产品线" prop="product_line">
                  <div v-if="formEdit.product_line != undefined">
                   <span>{{ formEdit.product_line.name }}<b>（产品线一旦创建不可更改！！）</b></span>
                  </div>
                </el-form-item></el-col>
              </el-row>
              <el-row :gutter="20">
                <el-col :span="8"><el-form-item label="级别" prop="level">
                  <el-select v-model="formEdit.level" placeholder="请选择级别">
                    <el-option
                      v-for="item in optionsLevel"
                      :key="item.value"
                      :label="item.label"
...
<script>
import {
  getOriginUnitProjectSubmit,
  createOriginUnitProjectSubmit,
  updateOriginUnitProjectSubmit,
  exportOriginUnitProjectSubmit,
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getOriginUnitProjectSubmit(this.params).then(
        res => {
    ...
    handleEdit(values) {
      ...
      this.formEdit.level = this.formEdit.level.id
    ...
    handleSubmitEdit() {
      this.$refs.handleFormEdit.validate(valid => {
        ...
        let attrStr
        console.log(data)
        const transFieldStr = ['product_line', 'creator', 'mistake_tag']
        for (attrStr in transFieldStr) {
          data[transFieldStr[attrStr]] = data[transFieldStr[attrStr]].id
        }
        console.log(data)
        updateOriginUnitProjectSubmit(id, data).then(
          () => {
            this.$notify({
              title: '更新成功',
              type: 'success',
    ...
    handleSubmitAdd() {
      ...
      createOriginUnitProjectSubmit(this.formAdd).then(
        () => {
          this.fetchData()
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportOriginUnitProjectSubmit(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportOriginUnitProjectSubmit(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportOriginUnitProjectSubmit(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkOriginUnitProjectSubmit(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectOriginUnitProjectSubmit(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',

<template>
  <div class="unitproject-confirm-container">
...
<script>
import {
  getUnitProjectConfirm,
  updateUnitProjectConfirm,
  exportUnitProjectConfirm,
  excelImportUnitProjectConfirm,
  checkUnitProjectConfirm,
  rejectUnitProjectConfirm,
  photoImportUnitProjectConfirm,
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getUnitProjectConfirm(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportUnitProjectConfirm(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportUnitProjectConfirm(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportUnitProjectConfirm(this.params).then(
              res => {
    ...
    handleSetTag() {
      ...
      if (this.params.allSelectTag === 1) {
        setTagUnitProjectConfirm(this.params).then(
          res => {
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagUnitProjectConfirm(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '设置成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              checkUnitProjectConfirm(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '确认成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectUnitProjectConfirm(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handleMemo(row) {
      this.$prompt('请输入备注', '添加备注', {
        ...
        ({ value }) => {
          ...
          updateUnitProjectConfirm(id, data).then(
            () => {
              this.$notify({
                title: '修改成功',
                type: 'success',

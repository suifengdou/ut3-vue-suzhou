  ...
  <div class="ori-invoice-submit-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.work_order.name }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="开票单号"
    ...
    <el-dialog
      ...
      :close-on-press-escape="false"
    >
      <el-form ref="invoiceIDForm" label-width="10%" :data="invoiceIDData">
        <hr>
        <el-form-item label="发票单号">
          <el-input
            v-model="invoiceIDData.invoice_id"
            type="textarea"
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getInvoiceListHandle(this.params).then(
        res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            exportInvoiceSubmit(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkInvoiceHandle(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectInvoiceHandle(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    fillInvoiceID(row, column, cell, event) {
      if (column.property === 'invoice_id') {
        this.InvoiceIDVisible = true
        this.invoiceIDData = row
      }
    },
    handleFillID() {
      this.InvoiceIDVisible = true
      this.invoiceIDData = this.formEdit
    },
    sumitInvoiceID() {
      console.log(this.invoiceIDData)
      const id = this.invoiceIDData.id
      const data = {
        invoice_id: this.invoiceIDData.invoice_id
      }
      updateInvoiceHandle(id, data).then(
        res => {
          this.$notify({
            title: '修改发票单号成功',
            type: 'success',

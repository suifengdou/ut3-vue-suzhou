...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getDeliverList(this.params).then(
        res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            exportDeliver(this.params).then(
              res => {
                res.data = res.data.map(item => {
                  return {
                    ...
                    地址: item.address,
                    手机: item.smartphone,
                    发货条件: item.condition_deliver,
                    优惠金额: item.discounts,
                    邮费: item.postage,
                    货品价格: item.goods_price,
                    货品总价: item.goods_amount,
    ...
    sumitInvoiceID() {
      ...
      const data = {
        logistics: this.deliverData.logistics,
        track_no: this.deliverData.track_no
      }
      console.log(data)

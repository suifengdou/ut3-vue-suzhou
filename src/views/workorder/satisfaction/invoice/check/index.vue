  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        getInvoiceCheckList(this.params).then(
          res => {
      ...
      handleSubmitEdit() {
        this.$refs.handleFormEdit.validate(valid => {
          ...
          updateInvoiceCheck(id, data).then(
            () => {
      ...
      handleSubmitAdd() {
        ...
        createInvoiceCheck(this.formAdd).then(
          () => {
            this.fetchData()
      ...
      importExcel() {
        ...
        excelImportInvoiceCheck(importformData, config).then(
          res => {
            this.$notify({
              title: '导入结果',
              message: res.data,
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              exportInvoiceCheck(this.params).then(
                res => {
      ...
      handleCheck() {
        ...
        if (this.params.allSelectTag === 1) {
          checkInvoiceCheck(this.params).then(
            res => {
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              instance.confirmButtonText = '执行中...'
              if (this.params.allSelectTag === 1) {
                rejectInvoiceCheck(this.params).then(
                  res => {
                    if (res.data.successful !== 0) {
                      this.$notify({
                        title: '取消成功',

...
export default {
  ...
  methods: {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportWorkOrderCreate(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
    ...
    handleFix() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        fixWorkOrderCreate(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '修复成功',

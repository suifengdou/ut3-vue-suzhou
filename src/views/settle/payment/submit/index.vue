              ...
              <el-tooltip class="item" effect="dark" content="点击展开操作列表，可执行对应操作" placement="top-start">
                <el-dropdown split-button type="primary" placement="bottom-end" trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown" trigger="click">
                    ...
                    <el-dropdown-item><el-button type="danger" icon="el-icon-check" size="mini" round @click="handleReject">驳回单据</el-button></el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </el-tooltip>
...
import {
  ...
  updatePaymentSubmit,
  exportPaymentSubmit,
  excelImportPaymentSubmit,
  checkPaymentSubmit,
  rejectPaymentSubmit,
  photoImportPaymentSubmit,
  setTagPaymentSubmit,
  resetTagPaymentSubmit,
} from '@/api/settle/payment/submit'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getPaymentSubmit(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportPaymentSubmit(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
    ...
    handlePhotoView(userValue) {
      ...
      getFileDetailsPayment(data).then(
        res => {
          this.$notify({
            title: '查询成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportPaymentSubmit(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportPaymentSubmit(this.params).then(
              res => {
    ...
    handleSetTag() {
      ...
      if (this.params.allSelectTag === 1) {
        setTagPaymentSubmit(this.params).then(
          res => {
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagPaymentSubmit(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '设置成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              checkPaymentSubmit(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '确认成功',
    ...
    handleReject() {
      ...
      this.$msgbox({
        ...
        message: h('p', null, [
          h('h3', { style: 'color: teal' }, '特别注意：'),
          h('hr', null, ''),
          h('span', null, '驳回项目成功则对应结算单会恢复对应未结算金额'),
          h('b',  { style: 'color: red' }, '结算单如果已审核则无法驳回！'),
          h('hr', null, '')
        ]),
        showCancelButton: true,
        confirmButtonText: '确定',
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPaymentSubmit(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handleMemo(row) {
      this.$prompt('请输入备注', '添加备注', {
        ...
        ({ value }) => {
          ...
          updatePaymentSubmit(id, data).then(
            () => {
              this.$notify({
                title: '修改成功',
                type: 'success',
    ...
    handleOAID(row) {
      this.$prompt('请输入OA编号', '添加OA编号', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        inputValue: row.oa_id,
        inputErrorMessage: '输入不能为空',
        inputValidator: (value) => {
        if(!value) {
          return '输入不能为空';

<template>
  <div class="phototypeexecuteprojectstatement-check-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button type="danger" size="mini" @click="handleCreatePaymentTotal(scope.row)">全额</el-button>
            <el-button type="danger" size="mini" @click="handleCreatePaymentSplit(scope.row)">拆分</el-button>
          </template>
        </el-table-column>
        <el-table-column
          label="结算金额"
...
<script>
import {
  getPhototypeStatementCheck,
  updatePhototypeStatementCheck,
  exportPhototypeStatementCheck,
  excelImportPhototypeStatementCheck,
  checkPhototypeStatementCheck,
  rejectPhototypeStatementCheck,
  photoImportPhototypeStatementCheck,
  setTagPhototypeStatementCheck,
  resetTagPhototypeStatementCheck,
  createTotalPhototypeStatementCheck,
  createSplitPhototypeStatementCheck,
  createCombinePhototypeStatementCheck
} from '@/api/statement/phototypestatement/check'
import { getLogPhototypeStatement } from '@/api/statement/phototypestatement/manage'
...
export default {
  name: 'checkPhototypeExecuteProjectStatement',
  ...
  methods: {
    fetchData() {
      ...
      getPhototypeStatementCheck(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportPhototypeStatementCheck(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportPhototypeStatementCheck(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportPhototypeStatementCheck(this.params).then(
              res => {
    ...
    handleSetTag() {
      ...
      if (this.params.allSelectTag === 1) {
        setTagPhototypeStatementCheck(this.params).then(
          res => {
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagPhototypeStatementCheck(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '设置成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              checkPhototypeStatementCheck(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '确认成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPhototypeStatementCheck(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handleMemo(row) {
      this.$prompt('请输入备注', '添加备注', {
        ...
        ({ value }) => {
          ...
          updatePhototypeStatementCheck(id, data).then(
            () => {
              this.$notify({
                title: '修改成功',
                type: 'success',
    ...
    // 全额创建付款单
    handleCreatePaymentTotal(row) {
      const h = this.$createElement
      const data = {
        id: row.id,
        pay_amount: row.settle_amount
      }
      this.$msgbox({
        title: '导出 Excel',
        message: h('p', null, [
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            createTotalPhototypeStatementCheck(data).then(
              res => {
                if (res.data.successful !== 0) {
                    this.$notify({
                      title: '创建成功',
    ...
    // 拆分创建付款单
    handleCreatePaymentSplit(row) {
      const h = this.$createElement
      this.$prompt('请输入付款金额', '添加付款金额', {
        title: '创建付款单',
        message: h('p', null, [
          h('h3', { style: 'color: teal' }, '特别注意：'),
          h('hr', null, ''),
          h('span', null, `结算未付余额：`),
          h('b', { style: 'color: red' }, `${row.remaining}`),
          h('hr', null, ''),
        ]),
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        inputValue: row.pay_amount,
        ...
        ({ value }) => {
          ...
          updatePhototypeStatementCheck(id, data).then(
            ...
            (error) => {
              this.$notify({
                title: '创建失败',
                message: `创建失败：${JSON.stringify(error.data)}`,
                type: 'error',
                offset: 70,
                duration: 0
              })

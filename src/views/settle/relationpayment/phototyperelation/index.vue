  ...
  <div class="phototypeexecuteprojectstatement-manage-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.origin_obj.name }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="源单金额"
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.origin_amount }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="付款单"
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.pay_obj.name }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="付款金额"
...
<script>
import {
  getPhototypeRelation,
  exportPhototypeRelation,
  getLogPhototypeRelation,
} from '@/api/settle/relationpayment/phototyperelation'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getPhototypeRelation(this.params).then(
        res => {
    ...
    importPhotoes() {
      ...
      this.tableLoading = true
      photoImportPhototypeRelation(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
    ...
    logView(userValue) {
      ...
      getLogPhototypeRelation(data).then(
        res => {
          this.$notify({
            title: '查询成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportPhototypeRelation(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportPhototypeRelation(this.params).then(
              res => {
    ...
    handleSetTag() {
      ...
      if (this.params.allSelectTag === 1) {
        setTagPhototypeRelation(this.params).then(
          res => {
    ...
    handleResetTag() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        resetTagPhototypeRelation(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '设置成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              checkPhototypeRelation(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '确认成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectPhototypeRelation(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
    ...
    handleMemo(row) {
      this.$prompt('请输入备注', '添加备注', {
        ...
        ({ value }) => {
          ...
          updatePhototypeRelation(id, data).then(
            () => {
              this.$notify({
                title: '修改成功',
                type: 'success',

<template>
  <div class="vipwechat-submit-container">
        ...
        <el-col :span="5" class="titleBar">
          <div class="grid-content bg-purple">
            <el-tooltip class="item" effect="dark" content="快捷搜索" placement="top-start">
              <el-input v-model="params.customer__name" class="grid-content bg-purple" placeholder="请输入用户电话" @keyup.enter.native="fetchData">
              ...
              <el-collapse-item>
                ...
                <div class="block">
                  <el-form ref="filterForm" :model="params" label-width="80px">
                    <el-row :gutter="20">
                      <el-col :span="8"><el-form-item label="服务账号" prop="specialist__name">
                        <el-input v-model="params.specialist__name" type="text" />
                      </el-form-item></el-col>
                      <el-col :span="8"><el-form-item label="服务账号手机" prop="specialist__smartphone">
                        <el-input v-model="params.specialist__smartphone" type="text" />
                      </el-form-item></el-col>
                      <el-col :span="6" />
                    </el-row>
                    <el-row :gutter="20">
                      <el-col :span="8"><el-form-item label="客户微信" prop="cs_wechat">
      ...
      >
        <el-card class="box-card">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8"><el-form-item label="服务账号" prop="specialist">
              <template>
                <el-select
                  v-model="formAdd.specialist"
                  filterable
                  default-first-option
                  remote
                  reserve-keyword
                  placeholder="请搜索并选择服务账号"
                  :remote-method="remoteMethodSpecialist"
                >
                  <el-option
                    v-for="item in optionsSpecialist"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
        ...
        <el-card class="box-card">
          ...
          <el-row :gutter="20">
            <el-col :span="8"><el-form-item label="客户微信" prop="cs_wechat">
              <el-input v-model="formAdd.cs_wechat" placeholder="请输入名称" />
            </el-form-item></el-col>
        ...
        <div class="handleFormEdit">
          ...
          >
            ...
            <el-card class="box-card">
              ...
              <el-row :gutter="20">
                <el-col :span="8"><el-form-item label="客户微信" prop="cs_wechat">
                  <el-input v-model="formEdit.cs_wechat" placeholder="请输入名称" />
                </el-form-item></el-col>
                <el-col :span="8"><el-form-item label="备注" prop="memo">
...
<script>
import {
  getVIPWechatMyselfList,
  createVIPWechatMyself,
  updateVIPWechatMyself,
  exportVIPWechatMyself,
  excelImportVIPWechatMyself,
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getVIPWechatMyselfList(this.params).then(
        res => {
    ...
    handleEdit(values) {
      ...
      this.optionsSpecialist = [{ label: this.formEdit.specialist.name, value: this.formEdit.specialist.id }]
      this.formEdit.specialist = this.formEdit.specialist.id
    ...
    handleSubmitEdit() {
      ...
      updateVIPWechatMyself(id, data).then(
        () => {
    ...
    handleSubmitAdd() {
      createVIPWechatMyself(this.formAdd).then(
        () => {
          this.fetchData()
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportVIPWechatMyself(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportVIPWechatMyself(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkVIPWechatMyself(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectVIPWechatMyself(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',
    ...
    remoteMethodSpecialist(query) {
      if (query !== '') {
        ...
        setTimeout(() => {
          ...
          const paramsSearch = {}
          paramsSearch.name = query
          getSpecialistList(paramsSearch).then(
            res => {
              this.optionsSpecialist = res.data.results.map(item => {
                return { label: item.name, value: item.id }
              })
            }
          )

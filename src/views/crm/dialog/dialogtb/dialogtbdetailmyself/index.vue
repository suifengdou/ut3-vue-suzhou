              ...
              <el-tooltip class="item" effect="dark" content="点击展开操作列表，可执行对应操作" placement="top-start">
                <el-dropdown split-button type="primary" placement="bottom-end" trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown" trigger="click">
                    <el-dropdown-item><el-button type="success" icon="el-icon-check" size="mini" round @click="handleCheck">常规审核审核</el-button></el-dropdown-item>
...
<script>
import {
  getDialogTBDetailMyselfList,
  createDialogTBDetailMyself,
  updateDialogTBDetailMyself,
  exportDialogTBDetailMyself,
  checkDialogTBDetailMyself,
  rejectDialogTBDetailMyself
} from '@/api/crm/dialog/taobao/dialogtbdetailmyself'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getDialogTBDetailMyselfList(this.params).then(
        res => {
    ...
    handleSubmitAdd() {
      ...
      createDialogTBDetailMyself(this.formAdd).then(
        () => {
          this.$notify({
            title: '创建成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            exportDialogTBDetailMyself(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkDialogTBDetailMyself(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectDialogTBDetailMyself(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',

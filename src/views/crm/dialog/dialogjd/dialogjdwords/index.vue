...
<script>
import {
  getDialogJDWordsList,
  createDialogJDWords,
  updateDialogJDWords,
  exportDialogJDWords,
  excelImportDialogJDWords,
  checkDialogJDWords,
  rejectDialogJDWords
} from '@/api/crm/dialog/jd/dialogjdwords'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getDialogJDWordsList(this.params).then(
        res => {
    ...
    handleSubmitEdit() {
      ...
      updateDialogJDWords(id, data).then(
        () => {
          this.$notify({
            title: '修改成功',
            type: 'success',
    ...
    handleSubmitAdd() {
      ...
      createDialogJDWords(this.formAdd).then(
        () => {
          this.$notify({
            title: '创建成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportDialogJDWords(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportDialogJDWords(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkDialogJDWords(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectDialogJDWords(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',

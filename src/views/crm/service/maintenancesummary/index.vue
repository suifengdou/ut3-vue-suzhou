  ...
  <div class="ori-order-container">
              ...
              <el-tooltip class="item" effect="dark" content="点击展开操作列表，可执行对应操作" placement="top-start">
                <el-dropdown split-button type="primary" placement="bottom-end" trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown" trigger="click">
                    <el-dropdown-item><el-button type="success" icon="el-icon-check" size="mini" round @click="handleCheck">重算</el-button></el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </el-tooltip>
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.repeat_found }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="当天二次维修量"
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.repeat_today }}</span>
          </template>
        </el-table-column>
...
<script>
import {
  getMaintenanceSummaryList,
  createMaintenanceSummary,
  updateMaintenanceSummary,
  exportMaintenanceSummary,
  excelImportMaintenanceSummary,
  checkMaintenanceSummary,
  fixMaintenanceSummary,
  rejectMaintenanceSummary
} from '@/api/crm/service/maintenancesummary/maintenancesummary'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getMaintenanceSummaryList(this.params).then(
        res => {
    ...
    handleSubmitEdit() {
      ...
      updateMaintenanceSummary(id, data).then(
        () => {
          this.$notify({
            title: '修改成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportMaintenanceSummary(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportMaintenanceSummary(this.params).then(
              res => {
                console.log(res)
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkMaintenanceSummary(this.params).then(
          res => {

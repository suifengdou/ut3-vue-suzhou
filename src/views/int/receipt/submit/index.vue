  ...
  <div class="receipt-submit-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button type="danger" size="mini" @click="handleConfirm(scope.row)">认领</el-button>
            <el-button type="danger" size="mini" @click="handleReset(scope.row)">重置</el-button>
          </template>
        </el-table-column>
        <el-table-column
          label="是否到账"
...
<script>
  import {
    getReceiptSubmitList,
    createReceiptSubmit,
    updateReceiptSubmit,
    exportReceiptSubmit,
    excelImportReceiptSubmit,
    checkReceiptSubmit,
    rejectReceiptSubmit,
    photoImportReceiptSubmit,
    confirmReceiptSubmit,
    resetReceiptSubmit
  } from '@/api/int/receipt/submit'
  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        getReceiptSubmitList(this.params).then(
          res => {
      ...
      handleSuggestion(row) {
        this.$prompt('请输入处理意见', '添加处理意见', {
          ...
          ({ value }) => {
            ...
            updateReceiptSubmit(id, data).then(
              () => {
                this.$notify({
                  title: '修改成功',
                  type: 'success',
      ...
      handleConfirm(row) {
        ...
        this.$msgbox({
          ...
          message: h('p', null, [
            ...
            h('hr', null, ''),
            h('span', null, '点击确认，设置当前领款人，只有领款人是自己才可以审核收款单。'),
            h('hr', null, ''),
          ]),
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              confirmReceiptSubmit(data).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '认领成功',
                      message: `认领成功条数：${res.data.successful}`,
                      type: 'success',
                      offset: 70,
                      duration: 3000
                    })
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              instance.confirmButtonLoading = true
              instance.confirmButtonText = '执行中...'
              resetReceiptSubmit(data).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '清空成功',
                      message: `清空成功条数：${res.data.successful}`,
                      type: 'success',
                      offset: 70,
                      duration: 3000
                    })
      ...
      importPhotoes() {
        ...
        this.tableLoading = true
        photoImportReceiptSubmit(importformData, config).then(
          res => {
            this.$notify({
              title: '导入结果',
              message: res.data,
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              excelImportReceiptSubmit(importformData, config).then(
                res => {
                  this.$notify({
                    title: '导入结果',
                    message: res.data,
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              instance.confirmButtonLoading = true
              instance.confirmButtonText = '执行中...'
              exportReceiptSubmit(this.params).then(
                res => {
      ...
      handleCheck() {
        ...
        if (this.params.allSelectTag === 1) {
          checkReceiptSubmit(this.params).then(
            res => {
              if (res.data.successful !== 0) {
                this.$notify({
                  title: '审核成功',
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              instance.confirmButtonText = '执行中...'
              if (this.params.allSelectTag === 1) {
                rejectReceiptSubmit(this.params).then(
                  res => {
                    if (res.data.successful !== 0) {
                      this.$notify({
                        title: '驳回成功',

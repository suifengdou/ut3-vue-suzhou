  ...
  <div class="goodsdetails-container">
    ...
    <div class="table-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.parts.name }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="原子件"
...
<script>
import { getGoodsDetails } from '@/api/bom/goods/goodsdetails'
import moment from 'moment'
export default {
  name: 'goodsdetails-container',
  data() {
    return {
      DataList: [],
      tableLoading: false,
  ...
  methods: {
    ...
    fetchData() {
      ...
      if (typeof (this.params.created_time) !== 'undefined') {
        if (this.params.create_timed.length === 2) {
          this.params.create_timed_after = moment.parseZone(this.params.created_time[0]).local().format('YYYY-MM-DD HH:MM:SS')
          this.params.create_timed_before = moment.parseZone(this.params.created_time[1]).local().format('YYYY-MM-DD HH:MM:SS')
        }
      }
      getGoodsDetails(this.params).then(
        res => {

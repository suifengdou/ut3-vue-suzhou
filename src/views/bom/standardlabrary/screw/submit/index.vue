...
import { 
  ...
  rejectScrewSubmit,
  fixScrewSubmit
  } from '@/api/bom/standardlabrary/screw/submit'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getScrewSubmit(this.params).then(
        res => {
    ...
    handleSubmitAdd() {
      ...
      createScrewSubmit(this.formAdd).then(
        () => {
          this.fetchData()
    ...
    handleSubmitEdit() {
      this.$refs.FormEdit.validate(valid => {
        ...
        console.log(data)
        updateScrewSubmit(id, data).then(
          () => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkScrewSubmit(this.params).then(
          res => {
    ...
    handleFix() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        fixScrewSubmit(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '修复成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            if (this.params.allSelectTag === 1) {
              rejectScrewSubmit(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            importScrewSubmit(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
